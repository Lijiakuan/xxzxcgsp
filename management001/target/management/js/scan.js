    ...
    $("#input-b9").fileinput('destroy').fileinput({
        ...
        var opt = {
            ...
            query:{
                ...
                productdesc:$("#txt_search_productdesc").val(),
                scanstatus:$("#txt_search_scanstatus").val(),
...
$(function () {
    ...
    var productdesc = $("#txt_search_productdesc");
    var scanstatus = $("#txt_search_scanstatus");
    ...
    //制片描述：常规、冰冻、会诊
    productdesc.append("<option></option>");
    productdesc.append("<option>常规</option>");
    productdesc.append("<option>冰冻</option>");
    productdesc.append("<option>会诊</option>");
    //是否完成扫描
    scanstatus.append("<option></option>");
    scanstatus.append("<option>未扫描</option>");
    scanstatus.append("<option>已扫描</option>");
    //患者列表
    $.ajax({
        type: "get",
...
function downloadFilebyAjax() {
    ...
    form.append($("<input></input>").attr("type", "hidden").attr("name", "productdesc").attr("value", productdesc));
    form.append($("<input></input>").attr("type", "hidden").attr("name", "scanstatus").attr("value", scanstatus));
    ...
    oTableInit.Init = function () {
        $('#tb_tasks').bootstrapTable({
            onCheck:function(row){
                if(rowNums.indexOf(row.productno)<0){
                    rowNums.push(row.productno);
                    queryResults001(rowNums)
                }
            },
            onUncheck:function(row){
                if(rowNums.indexOf(row.productno)>0){
                    rowNums.remove(row.productno);
                    queryResults001(rowNums)
                }
                debugger;
            },
            ...
            if(value != null){
                ...
                {field:'productstatus',title:'扫描状态',width:90,align:'left',sortable : true,formatter:function(value,row,index){
                    var temp_html="<span class='label label-danger'>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp未扫描&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</span>";
                    if(value == "已扫描" || value == "绑定数字切片"){
                        temp_html = "<span class='label label-success'>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp已扫描&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</span>";
                    }
                return temp_html;
                }},
